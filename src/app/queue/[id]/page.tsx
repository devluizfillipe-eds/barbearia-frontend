...
export default function QueueStatusPage() {
  ...
  return (
    <main className="min-h-screen bg-background p-4 flex flex-col items-center justify-center relative overflow-hidden">
      ...
      <Card className="w-full max-w-md border-2 border-primary/40 shadow-2xl shadow-primary/20 bg-card/95 backdrop-blur-sm rounded-2xl">
        <CardHeader className="text-center pb-2">
          <CardTitle className="text-2xl text-foreground">Status da Fila</CardTitle>
        ...
        <CardContent className="flex flex-col items-center space-y-8 pt-6">
          ...
          <div className="relative">
            <div className={cn("h-32 w-32 rounded-full border-4 flex items-center justify-center", 
              queueItem.status === QueueStatus.WAITING ? "border-primary/70 bg-primary/20" :
              queueItem.status === QueueStatus.IN_PROGRESS ? "border-green-500/70 bg-green-500/20" :
              "border-muted bg-muted/10"
            )}>
              {queueItem.status === QueueStatus.WAITING && <Clock className="h-16 w-16 text-primary animate-pulse" />}
          ...
          <Button variant="outline" className="w-full border-2 border-primary/40 hover:bg-primary/10 hover:border-primary rounded-full font-medium" onClick={() => router.push('/')}>
            Voltar ao Início
          </Button>
        </CardContent>
      </Card>
